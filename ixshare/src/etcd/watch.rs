...
impl Watcher {
    ...
    pub fn ParseEvent(&self, e: &Event) -> Result<Option<WatchEvent>> {
        ...
        let curObj: Option<DataObject<Value>> = if e.event_type() == etcd_client::EventType::Delete
        {
            None
        } else {
            let obj = Object::Decode(kv.value())?;
            Some(DataObject::<Value>::NewFromObject(
                &obj,
                0,
                kv.mod_revision(),
                0,
        ...
        let oldObj: Option<DataObject<Value>> = match e.prev_kv() {
            None => None,
            Some(pkv) => {
    ...
    pub fn Filter(&self, obj: &DataObject<Value>) -> bool {

...
impl EtcdStore {
    ...
    pub async fn GetRev(&self, obj: &DataObject<Value>) -> Result<i64> {
        let key = obj.StoreKey();
        let preparedKey = self.PrepareKey(&key)?;
        let resp = self
            .client
            .lock()
            .await
            .put(preparedKey, obj.Encode()?, None)
            .await?;
        return Ok(resp.header().unwrap().revision());
    }
...
impl EtcdStore {
    ...
    pub async fn LeaseKeepalive(&self, leaseId: i64) -> Result<()> {
        ...
        return Err(Error::CommonError(format!(
            "LeaseKeepalive stream read fail"
        )));
    }

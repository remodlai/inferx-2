...
pub trait BackendStore: Sync + Send + Debug {
    async fn Get(&self, key: &str, minRevision: i64) -> Result<Option<DataObject<Value>>>;
...
impl CacheStore {
    ...
    pub fn Add(&self, obj: &DataObject<Value>) -> Result<()> {
        return self.write().unwrap().Add(obj, false);
    }

    pub fn Update(&self, obj: &DataObject<Value>) -> Result<()> {
        return self.write().unwrap().Update(obj, false);
    }
    ...
    pub async fn Get(
        ...
        let objKey = tenant.to_string() + "/" + namespace + "/" + name;
        if revision == -1 {
    ...
    pub async fn List(
        ...
        let mut objs: Vec<DataObject<Value>> = Vec::new();

        let objKeyPrefix = if tenant.len() == 0 {
            "".to_owned()
        ...
        for (_, obj) in &inner.cacheStore {
            if obj.Key().starts_with(&objKeyPrefix) && opts.predicate.Match(obj)? {
                objs.push(obj.clone());
            }
        }
    ...
    pub fn GetObject(&self, key: &str) -> Option<DataObject<Value>> {
...
pub struct CacheStoreInner {
    ...
    pub cacheStore: BTreeMap<String, DataObject<Value>>,
...
impl CacheStoreInner {
    ...
    pub fn Refresh(&mut self, objs: &[DataObject<Value>]) -> Result<()> {
    ...
    pub fn GetByKey(&self, key: &str) -> Option<DataObject<Value>> {
        match self.cacheStore.get(key) {
            None => return None,
